...
// Função para ordenar a lista de pokemons por id usando o algoritmo Bubble Sort
// Em caso de empate, ordenar por nome
void bubbleSort(PokeList *pokeList) {
  bool trocou;
  int n = pokeList->tamanho;

  do {
    trocou = false;
    for (int i = 0; i < n - 1; i++) {
      movimentacoes++;
      int id1 = atoi(pokeList->pokemon[i].id);
      int id2 = atoi(pokeList->pokemon[i + 1].id);

      if (id1 > id2 || (id1 == id2 && strcmp(pokeList->pokemon[i].name, pokeList->pokemon[i + 1].name) > 0)) {
        // Trocar os pokemons
        pokemon temp = pokeList->pokemon[i];
        pokeList->pokemon[i] = pokeList->pokemon[i + 1];
        pokeList->pokemon[i + 1] = temp;
    ...
    n--; // Otimização: reduzir o número de iterações após cada passagem
  } while (trocou);
}
...
int main(void) {
  ...
  FILE *logFile;
  logFile = fopen("853733_bolha.txt", "w");
  if (logFile == NULL) {
