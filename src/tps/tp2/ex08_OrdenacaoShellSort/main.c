...
// ------ Funcoes para ordenar a lista de pokemons por weight usando o algoritmo Shellsort
...
// Shellsort function to sort the list of Pokémon
void shellSortPokemon(PokeList *pokeList) {
  ...
  for (gap = n / 2; gap > 0; gap /= 2) {
    for (i = gap; i < n; i++) {
      temp = pokeList->pokemon[i];
      movimentacoes++;
      for (j = i; j >= gap && comparePokemon(&pokeList->pokemon[j - gap], &temp) > 0; j -= gap) {
        pokeList->pokemon[j] = pokeList->pokemon[j - gap];
        movimentacoes++;
      }
      pokeList->pokemon[j] = temp;
      movimentacoes++;
    }
  }
...
int main(void) {
  ...
  FILE *logFile;
  logFile = fopen("853733_shellsort.txt", "w");
  if (logFile == NULL) {
