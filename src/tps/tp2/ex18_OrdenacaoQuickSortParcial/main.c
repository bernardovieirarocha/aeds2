...
// ------ Funcoes para ordenar a lista de pokemons por generation usando o algoritmo QuickSort de forma Parcial
// Em caso de empate, ordenar por nome

// Function to swap two Pokémon
void swap(pokemon *a, pokemon *b) {
    pokemon temp = *a;
    *a = *b;
    *b = temp;
    movimentacoes += 3; // Incrementa ao mover elementos
}
...
void quickSortPartial(pokemon *arr, int low, int high, int k) {
    if (low < high) {
        int pi = partition(arr, low, high);

        if (pi - low > k) {
            quickSortPartial(arr, low, pi - 1, k);
        }
        if (high - pi > k) {
            quickSortPartial(arr, pi + 1, high, k);
        }
    }
}
...
int main(void) {
  ...
  // QuickSort Parcial para ordenar a lista de pokemons
  int k = 10;
  quickSortPartial(pokeList->pokemon, 0, pokeList->tamanho - 1, k);
  ...
  FILE *logFile;
  logFile = fopen("853733_quicksortParcial.txt", "w");
  if (logFile == NULL) {
