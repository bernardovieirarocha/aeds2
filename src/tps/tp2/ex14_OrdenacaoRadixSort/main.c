...
// Função para encontrar o tamanho máximo da string em 'abilities'
int getMaxAbilityLength(PokeList *pokeList) {
  int maxLen = 0;
  for (int i = 0; i < pokeList->tamanho; i++) {
    int len = strlen(pokeList->pokemon[i].abilities);
    if (len > maxLen) {
      maxLen = len;
    }
  }
...
// Função auxiliar para fazer o counting sort com base em um determinado caractere de abilities
void countingSortByChar(PokeList *pokeList, int charPosition) {
  pokemon *output = (pokemon *)malloc(pokeList->tamanho * sizeof(pokemon));
  int count[257] = {0}; // 256 for ASCII and 1 for '\0' (end of string)

  // Contar a ocorrência de cada caractere na posição charPosition
  for (int i = 0; i < pokeList->tamanho; i++) {
    int charIndex = (charPosition < strlen(pokeList->pokemon[i].abilities)) ? (unsigned char)pokeList->pokemon[i].abilities[charPosition] : 0;
    count[charIndex + 1]++;  // +1 to account for 0 as null terminator
  }

  // Modificar count[i] para conter as posições de saída reais
  for (int i = 1; i < 257; i++) {
    count[i] += count[i - 1];
  }

  // Construir o array de saída, garantindo estabilidade
  for (int i = pokeList->tamanho - 1; i >= 0; i--) {
    int charIndex = (charPosition < strlen(pokeList->pokemon[i].abilities)) ? (unsigned char)pokeList->pokemon[i].abilities[charPosition] : 0;
    output[count[charIndex] - 1] = pokeList->pokemon[i];
    count[charIndex]--;
  }

  // Copiar os elementos ordenados de volta para a lista original
  for (int i = 0; i < pokeList->tamanho; i++) {
    pokeList->pokemon[i] = output[i];
  }

  free(output);
}

// Função para realizar o Radix Sort com base no atributo abilities
void radixSort(PokeList *pokeList) {
    int maxLen = getMaxAbilityLength(pokeList);

    // Perform counting sort for each position of characters in abilities
    for (int pos = maxLen - 1; pos >= 0; pos--) {
        countingSortByChar(pokeList, pos);
    }

    // Ensure sorting by name for Pokémon with equal abilities
    for (int i = 0; i < pokeList->tamanho - 1; i++) {
        for (int j = 0; j < pokeList->tamanho - i - 1; j++) {
            // Compare abilities first
            if (strcmp(pokeList->pokemon[j].abilities, pokeList->pokemon[j + 1].abilities) == 0) {
                // If abilities are the same, compare names as secondary criterion
                if (strcmp(pokeList->pokemon[j].name, pokeList->pokemon[j + 1].name) > 0) {
                    pokemon temp = pokeList->pokemon[j];
                    pokeList->pokemon[j] = pokeList->pokemon[j + 1];
                    pokeList->pokemon[j + 1] = temp;
                }
            }
        }
    }
