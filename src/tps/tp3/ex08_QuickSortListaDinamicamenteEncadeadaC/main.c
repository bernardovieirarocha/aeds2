...
ListaDuplamenteEncadeada  * criarLista(void) {
...
// Adicionar um pokemon na lista Duplamente Encadeada
void addPokemonListaDuplamenteEncadeada(ListaDuplamenteEncadeada *lista, pokemon *p) {
  Celula *celula = criarCelula(p);
  if (lista->tamanho == 0) {
    lista->inicio = celula;
    lista->fim = celula;
  } else {
    lista->fim->prox = celula;
...
// ------ Funcoes para ordenar a lista de pokemons por generation usando o algoritmo QuickSort
// Em caso de empate, ordenar por nome

// Function to swap two Pokémon na ListaDuplamenteEncadeada
...
// Partition function for QuickSort na ListaDuplamenteEncadeada
Celula * partitionListaDuplamenteEncadeada(ListaDuplamenteEncadeada *lista, Celula *low, Celula *high) {
    pokemon pivot = *high->pokemon;
    Celula *i = low->ant;
 
    for (Celula *j = low; j != high; j = j->prox) {
        comparacoes++;
        if (getGeneration(j->pokemon) < pivot.generation ||
            (getGeneration(j->pokemon) == pivot.generation && strcmp(getName(j->pokemon), pivot.name) < 0)) {
            i = (i == NULL) ? low : i->prox;
            swap(i->pokemon, j->pokemon);
        }
    }
    i = (i == NULL) ? low : i->prox;
    swap(i->pokemon, high->pokemon);
    return i;
}
// QuickSort function for ListaDuplamenteEncadeada
void quickSortListaDuplamenteEncadeada(ListaDuplamenteEncadeada *lista, Celula *low, Celula *high) {
    if (high != NULL && low != high && low != high->prox) {
        Celula *pi = partitionListaDuplamenteEncadeada(lista, low, high);
        quickSortListaDuplamenteEncadeada(lista, low, pi->ant);
        quickSortListaDuplamenteEncadeada(lista, pi->prox, high);
    }
}

void printListaDuplamenteEncadeada(ListaDuplamenteEncadeada *lista) {
  Celula *celula = lista->inicio;
  while (celula != NULL) {
    printPokemon(celula->pokemon);
    celula = celula->prox;
  }
}

void freeListaDuplamenteEncadeada(ListaDuplamenteEncadeada *lista) {
  Celula *celula = lista->inicio;
  while (celula != NULL) {
    Celula *temp = celula;
    celula = celula->prox;
...
int main(void) {
  ...
  ListaDuplamenteEncadeada *pokeList = criarLista();
  ...
  while (strcmp(id, "FIM") != 0 && strcmp(id, "0") != 0) {
    ...
    if (poke != NULL) {
      addPokemonListaDuplamenteEncadeada(pokeList, poke);
    }
    scanf("%s", id);
  }
  ...
  quickSortListaDuplamenteEncadeada(pokeList, pokeList->inicio, pokeList->fim);

  // Finaliza a medição do tempo
  clock_t end_time = clock();
  double execution_time = ((double)(end_time - start_time)) / CLOCKS_PER_SEC;

  // Printar a lista de pokemons ordenada na lista duplamente encadeada
  printListaDuplamenteEncadeada(pokeList);
  ...
  // Desalocar a lista de pokemons
  freeListaDuplamenteEncadeada(pokeList);
  return 0;
}