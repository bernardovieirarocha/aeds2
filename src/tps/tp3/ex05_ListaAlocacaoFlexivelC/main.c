...
struct PokeListaEncadeada {
  Celula *primeiro;
  Celula *ultimo;
} typedef PokeListaEncadeada;
...
Celula *NovoCelula(pokemon *poke) {
  Celula *newCelula = (Celula *)malloc(1 * sizeof(Celula));
  newCelula->pokemon = poke;
  newCelula->proximo = NULL;
  return newCelula;
}

int getSize(PokeListaEncadeada *lista) {
  Celula *aux = lista->primeiro->proximo;
  int cont = 0;
  while (aux != NULL) {
    cont++;
    aux = aux->proximo;
...
PokeListaEncadeada *criarPokeListaEncadeada() {
  PokeListaEncadeada *lista =
      (PokeListaEncadeada *)malloc(sizeof(PokeListaEncadeada));
  lista->primeiro = (Celula *)malloc(sizeof(Celula)); // Cria o nó cabeça
  lista->primeiro->proximo = NULL;
  lista->ultimo = lista->primeiro;
  return lista;
}

void inserirListaFIM(PokeListaEncadeada *lista, pokemon *poke) {
  Celula *novaCelula = NovoCelula(poke);
  lista->ultimo->proximo = novaCelula;
  lista->ultimo = novaCelula;
}

void inserirNoInicio(PokeListaEncadeada *lista, pokemon *poke) {
  Celula *novaCel = NovoCelula(poke);
  novaCel->proximo = lista->primeiro->proximo;
  lista->primeiro->proximo = novaCel;
  if (lista->ultimo == lista->primeiro) {
    lista->ultimo = novaCel;
  } 
}

// void inserirNaPos(PokeListaEncadeada *lista, pokemon *poke, int pos) {
//   Celula *ant = lista->primeiro;

//   // Acho o lugar logo antes de onde quero inserir
//   for (int i = 1; i < pos && ant->proximo != NULL; i++) {
//     ant = ant->proximo;
//   }
//   if (ant == lista->ultimo) {
//     inserirListaFIM(lista, poke);
//     return;
//   }
//   Celula *novaCel = NovoCelula(poke);
//   novaCel->proximo = ant->proximo;
//   ant->proximo = novaCel;
// }
...
Celula *removerInicio(PokeListaEncadeada *lista) {
  if (lista->primeiro->proximo == NULL) {
    printf("Lista Vazia!");
    return NULL;
  }
  Celula *temp = lista->primeiro->proximo;
  pokemon *poke = temp->pokemon;
  lista->primeiro->proximo = temp->proximo;
  if (lista->primeiro->proximo == NULL) {
    lista->ultimo = lista->primeiro; // Atualiza a lista caso ela fique vazia.
  }
  return temp;
}

Celula *removerFim(PokeListaEncadeada *lista) {
  if (lista->primeiro->proximo == NULL) {
    printf("Lista Vazia!");
    return NULL;
  }
  Celula *ant = lista->primeiro;
  while (ant->proximo != lista->ultimo) {
    ant = ant->proximo;
  }

  Celula *removido = lista->ultimo;
  lista->ultimo = ant;
  lista->ultimo->proximo = NULL;
  return removido;
}

// Celula *removerNaPosicao(PokeListaEncadeada *lista, int pos) {
//   Celula *ant = lista->primeiro;
//   for (int i = 1; i < pos && ant->proximo != NULL; i++) {
//     ant = ant->proximo;
//   }
...
Celula * removerNaPosicao(PokeListaEncadeada *lista, int pos) {
  Celula * vazio = NovoCelula(NULL);
  if (pos < 0 || pos >= getSize(lista)) {
    return vazio;
  }
  if (pos == 0) {
    return removerInicio(lista);
  } 
  if (pos == getSize(lista) - 1) {
...
void exibirLista(PokeListaEncadeada *lista) {
  Celula *aux = lista->primeiro->proximo;
  int cont = 0;
  while (aux != NULL) {
    printf("[%d] ", cont++);
    printPokemon(aux->pokemon);
    aux = aux->proximo;
  }
}
...
int main(void) {
  ...
  PokeListaEncadeada *pokeLista = criarPokeListaEncadeada();

  char id[4];
  scanf("%s", id);
  while (strcmp(id, "FIM") != 0 && strcmp(id, "0") != 0) {
    inserirListaFIM(pokeLista, procurarPokemon(pokedex, id));
    scanf("%s", id);
  }
  ...
  for (int i = 0; i < qtd_operacoes; i++) {
    ...
    if (strcmp(operacao, "II") == 0) {
      char id[4];
      scanf("%s", id);
      inserirNoInicio(pokeLista, procurarPokemon(pokedex, id));
      ...
      int pos;
      scanf("%d %s", &pos, id);
      inserirNaPos(pokeLista, procurarPokemon(pokedex, id), pos);
    } else if (strcmp(operacao, "RI") == 0) {
      Celula *removido = removerInicio(pokeLista);
      printf("(R) %s\n", getName(removido->pokemon));
      if (removido != NULL) {
        free(removido);
      }
    } else if (strcmp(operacao, "RF") == 0) {
      Celula *removido = removerFim(pokeLista);
      printf("(R) %s\n", getName(removido->pokemon));
      if (removido != NULL) {
        free(removido);
      }
      ...
      int pos;
      scanf("%d", &pos);
      Celula *removido = removerNaPosicao(pokeLista, pos);
      printf("(R) %s\n", getName(removido->pokemon));
      if (removido != NULL) {
        free(removido);
      }
