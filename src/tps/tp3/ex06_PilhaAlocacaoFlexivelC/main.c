...
struct PokePilhaFlexivel
{
  Celula *topo;
  int tamanho;
} typedef PokePilhaFlexivel;
...
PokePilhaFlexivel * createPokePilhaFlexivel() {
  PokePilhaFlexivel *p = (PokePilhaFlexivel *)malloc(sizeof(PokePilhaFlexivel));
  if (p == NULL) {
    printf("Memory allocation error\n");
    return NULL;
  }
...
Celula *createCelula(pokemon *poke) {
...
void empilharPokeFlexivel(PokePilhaFlexivel *p, pokemon *poke) {
  Celula *c = createCelula(poke);
  c->pokemon = poke;
  c->prox = p->topo;
  p->topo = c;
  p->tamanho++;
...
pokemon* desempilharPokeFlexivel(PokePilhaFlexivel *p) {
  if (p->topo == NULL) {
    printf("Pilha vazia\n");
    return NULL;
  }
  Celula *c = p->topo;
  pokemon *removido = c->pokemon;
  p->topo = c->prox;
  free(c);
  p->tamanho--;
  return removido;
...
bool isEmptyPokeFlexivel(PokePilhaFlexivel *p) {
  return p->topo == NULL;
}

pokemon* topoPokeFlexivel(PokePilhaFlexivel *p) {
  if (p->topo == NULL) {
    printf("Pilha vazia\n");
    return NULL;
  }
...
void printPokePilhaFlexivel(PokePilhaFlexivel *p) {
  Celula *c = p->topo;
  int contador = 0;
  while (c != NULL) {
    printf("[%d] ", contador);
    printPokemon(c->pokemon);
    c = c->prox;
    contador++;
  }
}
// Inverte usando pilha auxiliar.
void printPokePilhaFlexivelInversa(PokePilhaFlexivel *p) {
    if (isEmptyPokeFlexivel(p)) {
        printf("A pilha está vazia.\n");
        return;
    }

    PokePilhaFlexivel *auxPilha = createPokePilhaFlexivel();
    Celula *atual = p->topo;

    while (atual != NULL) {
        empilharPokeFlexivel(auxPilha, atual->pokemon);
        atual = atual->prox;
    }
    int contador = 0;
    while (!isEmptyPokeFlexivel(auxPilha)) {
        printf("[%d] ", contador);
        pokemon *poke = desempilharPokeFlexivel(auxPilha);
        printPokemon(poke);
        contador++;
    }
...
int main(void) {
  ...
  PokePilhaFlexivel *pokePilha = createPokePilhaFlexivel();


  char id[4];
  scanf("%s", id);
  while (strcmp(id, "FIM") != 0 && strcmp(id, "0") != 0) {
    empilharPokeFlexivel(pokePilha, procurarPokemon(pokedex, id));
    scanf("%s", id);
  }
  ...
  for (int i = 0; i < qtd_operacoes; i++) {
    ...
    if (strcmp(operacao, "I") == 0) {
      ...
      int posicao;
      pokemon *poke = desempilharPokeFlexivel(pokePilha);
      printf("(R) %s\n", getName(poke));
    }
  }

  // Printar a lista de pokemons ordenada
  printPokePilhaFlexivelInversa(pokePilha);
