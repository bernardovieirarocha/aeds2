...
// Funcao para desenfileirar um pokemon
pokemon *desenfileirar(PokeFila *pokeFila) {
  if (pokeFila->inicio == NULL) {
    return NULL;
  }

  Celula *aux = pokeFila->inicio;
  pokemon *poke = &aux->pokemon;
  pokeFila->inicio = aux->prox;
  free(aux);
  pokeFila->tamanho--;
...
// Funcao para criar uma pilha de pokemons com maximo de 5 pokemons
PokeFila *criarPokeFila(void) {
  PokeFila *pokeFila = (PokeFila *)malloc(sizeof(PokeFila));
  if (pokeFila == NULL) {
    printf("Memory allocation error\n");
    exit(1);
  }
  pokeFila->inicio = NULL;
  pokeFila->fim = NULL;
  pokeFila->tamanho = 0;
  return pokeFila;
}

// Funcao para enfileirar um pokemon de maximo 5 pokemons
void enfileirar(PokeFila *pokeFila, pokemon poke) {
  if (pokeFila->tamanho == TAMANHO_FILA) {
    desenfileirar(pokeFila);
  }
  ...
  if (pokeFila->inicio == NULL) {
    pokeFila->inicio = nova;
  } else {
    pokeFila->fim->prox = nova;
  }
  pokeFila->fim = nova;
  pokeFila->tamanho++;
}
...
// Funcao para calcular a media de taxa de captura dos pokemons
int calcularMediaCaptureRate(PokeFila *pokeFila) {
  if (pokeFila->tamanho == 0) {
    return 0;
  }
  ...
  Celula *aux = pokeFila->inicio;
  while (aux != NULL) {
    soma += getCaptureRate(&aux->pokemon);
    aux = aux->prox;
  }

  return (int ) round( (double) soma /  (double) pokeFila->tamanho);
}

// Funcao para printar a fila de pokemons
void printFila(PokeFila *pokeFila) {
  Celula *aux = pokeFila->inicio;
  int contador = 0;
  while (aux != NULL) {
    printf("[%d] ", contador);
...
int main(void) {
  ...
  PokeFila *pokeFila = criarPokeFila();
